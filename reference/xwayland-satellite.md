...
xsettingsd is a daemon that can be used to control settings in X11 applications through the use of the XSETTINGS protocol. It is a simple program that can be used to control a variety of settings, including font settings, icon settings, and sound settings.
...
The format for the configuration file is simple. Setting names and their corresponding values are whitespace-separated, with at most one setting per line. *Example Format (from .xsettingsd man page):*
...
/path/to/xsettingsd &
```
xsettingsd.service.in is a systemd unit file that gets installed when using CMake. To enable it:
```
systemctl --user enable xsettingsd.service
```
If the XFixes header and library are present when xsettingsd is compiled, xsettingsd will use ClientDisconnectMode to indicate that the X server can be auto-terminated if xsettingsd is the last connected client (which can be useful when running Xwayland in "lazy mode").
Settings
